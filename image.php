	...
	<div id="main" class="col-sm-8 clearfix" role="main">
		...
		<article id="post-<?php the_ID(); ?>" <?php post_class('clearfix'); ?> role="article" itemscope itemtype="http://schema.org/BlogPosting">
			...
			<header>
				...
				<p class="meta"><?php _e("Posted", "jwdmc"); ?> <time datetime="<?php echo the_time('Y-m-j'); ?>" pubdate><?php the_time(); ?></time> <?php _e("by", "jwdmc"); ?> <?php the_author_posts_link(); ?>.</p>
...
<div id="sidebar1" class="col col-lg-4 fluid-sidebar sidebar" role="complementary">
	...
	<div class="well">
		...
		<h3><?php _e("Image metadata","jwdmc"); ?></h3>
	...
	if ( $imgmeta['image_meta']['created_timestamp'] ) {
		echo __("Date Taken","jwdmc") . ": " . date("d-M-Y H:i:s", $imgmeta['image_meta']['created_timestamp'])."<br />"; }
		if ( $imgmeta['image_meta']['copyright'] ) {
			echo __("Copyright","jwdmc") . ": " . $imgmeta['image_meta']['copyright']."<br />"; }
			if ( $imgmeta['image_meta']['credit'] ) {
				echo __("Credit","jwdmc") . ": " . $imgmeta['image_meta']['credit']."<br />"; }
				if ( $imgmeta['image_meta']['title'] ) {
					echo __("Title","jwdmc") . ": " . $imgmeta['image_meta']['title']."<br />"; }
					if ( $imgmeta['image_meta']['caption'] ) {
						echo __("Caption","jwdmc") . ": " . $imgmeta['image_meta']['caption']."<br />"; }
						if ( $imgmeta['image_meta']['camera'] ) {
							echo __("Camera","jwdmc") . ": " . $imgmeta['image_meta']['camera']."<br />"; }
							if ( $imgmeta['image_meta']['focal_length'] ) {
								echo __("Focal Length","jwdmc") . ": " . $imgmeta['image_meta']['focal_length']."mm<br />"; }
								if ( $imgmeta['image_meta']['aperture'] ) {
									echo __("Aperture","jwdmc") . ": f/" . $imgmeta['image_meta']['aperture']."<br />"; }
									if ( $imgmeta['image_meta']['iso'] ) {
										echo __("ISO","jwdmc") . ": " . $imgmeta['image_meta']['iso']."<br />"; }
										if ( $pshutter ) {
											echo __("Shutter Speed","jwdmc") . ": " . $pshutter . "<br />"; }
											?>
										</div>
